...
Link to Docs: [Kubernetes Docs](https://kubernetes.io/docs/home/)
...
TechWorld with Nana: [Youtube](https://www.youtube.com/watch?v=s_o8dwzRlu4&ab_channel=TechWorldwithNana)
...
- **Secret**: _Used to store secret data, like credentials, certificates, etc_. All data inside this configuration are encoded in base64, but it still not secure enough. By default, **the built-in security mechanism is not enabled!**, read the docs for more info ([Docs about Secrets](https://kubernetes.io/docs/concepts/configuration/secret/)). So the Secret are meant to be encrypted using a Third-Party API or Cloud Providers, since K8's doesn't encrypt them automatically. You them reference/connect a Secret in a Deployment/Pod, so that they can use that information/data
...
**OBS:** Kubernetes doesn't manage any data persistence! You must manage the persistent data youself (Backup, Restoration, etc...)
